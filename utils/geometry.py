...
def gen_normal(mean, std, size, device, min, max):
    n = size[0]
    t = torch.normal(mean=mean, std=std, size=size, device=device)
    t_filtered = t[(t >= min) & (t <= max)]
    while t_filtered.shape[0] < n:
        re = n - t_filtered.shape[0]
        t_add = torch.normal(mean=mean, std=std, size=(re, 1), device=device)
        t_add_filtered = t_add[(t_add >= min) & (t_add <= max)]
        t_filtered = torch.concat((t_filtered, t_add_filtered), dim=0)
    return t_filtered


def generate_random_translation(n=1, device="cpu"):
    t_x = gen_normal(0.0, 0.04, (n, 1), device, -0.1, 0.1)  # [-0.1, 0.1]
    t_y = gen_normal(0.0, 0.04, (n, 1), device, -0.1, 0.1)  # [-0.1, 0.1]
    epsilon = 1e-10
    t_z = gen_normal(0.7, 0.2, (n, 1), device, epsilon, 1)  # [epsilon, 1]
    t_xyz = torch.stack((t_x, t_y, t_z), dim=1)
    return t_xyz
