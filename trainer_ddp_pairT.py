...
from dataset import TRAINING_CATEGORIES
from dataset.util import get_dataloader
from models.models import RelPose
from utils import get_permutations, make_visualization_n, measure_model_profile
...
def get_parser():
    ...
    parser.add_argument("--num_iterations", type=int, default=1600_000)
    ...
    parser.add_argument("--dataset", type=str, default="co3d_pt")
...
class Trainer(object):
    def __init__(self, args) -> None:
        ...
        # DDP Initialization
        print('===================')
        print('number of gpus: {}'.format(self.num_gpus))
        ...
        if self.rank == 0:
            if not osp.exists(args.resume):
                ...
                shutil.copytree("t-graph", osp.join(self.output_dir, "t-graph"))
        ...
        while self.iteration < self.num_iterations:
            for batch in self.dataloader:
                ...
                with torch.autocast(
                    ...
                    truth_graph = batch["translation_pair"].to(self.device, non_blocking=True)

                    if self.random_num_images:  # control dynamic length of each sequence
                    ...
                    # turn on when measure the flops
                    # if self.epoch == 0 and self.iteration == 0:
                    #     sample_inputs = (relative_rotations, None, images, crop_params, False, False)
                    #     flops, params, mem = measure_model_profile(self.net, sample_inputs)
                    #     print(f"[????] GFLOPs: {flops:.2f}, Params: {params:.2f}M, Max Memory: {mem:.2f}MB")
                    # exit()
                    ...
                    # print('################# print gpu information ################')
                    for i in range(deviceCount):
                        handle = nvidia_smi.nvmlDeviceGetHandleByIndex(i)
                        util = nvidia_smi.nvmlDeviceGetUtilizationRates(handle)
                        mem = nvidia_smi.nvmlDeviceGetMemoryInfo(handle)
                        print(
                            f"|Device {i}| Mem Free: {mem.free / 1024 ** 2:5.2f}MB / {mem.total / 1024 ** 2:5.2f}MB | gpu-util: {util.gpu / 100:3.1%} | gpu-mem: {util.memory / 100:3.1%} |")
                    ...
                    n_c = len(get_permutations(n))
                    predicted_translations_graph = predicted_translations_graph.reshape(
                        num_batches * n_c * 2, 3
                    )

                    # add mask
                    mask = torch.ones_like(truth_graph, device=self.device)
                    num = 0
                    for b in range(num_batches):
                        for p in range(n_c):
                            z1 = truth_graph[b][p][0][2]
                            z2 = truth_graph[b][p][1][2]
                            if z1 <= 0 or z2 <= 0:
                                num += 1
                                mask[b, p, :, :] = 0
                    mask = mask.reshape(num_batches * n_c * 2, 3)

                    truth = truth.reshape(num_batches * n, 3)
                    truth_graph = truth_graph.reshape(num_batches * n_c * 2, 3)
                    l1 = L1Loss()

                    # balance the translation loss scale
                    loss_trans = l1(predicted_translations, truth)
                    loss_trans_graph = l1(predicted_translations_graph * mask, truth_graph * mask) / (
                                n_c * 2) * n

                factor = 1.0
                loss = loss_rot + loss_trans + loss_trans_graph * factor
                ...
                if self.rank == 0:
                    ...
                    if self.iteration % 20 == 0:
                        self.log_info(loss, loss_rot, loss_trans, loss_trans_graph)
                        ...
                        self.writer.add_scalar(
                            "Loss/translations_graph", loss_trans_graph.item(), self.iteration  # add t_graph loss
                        )
                    ...
                    if self.iteration % self.interval_visualize == 0 and n < 5:
                        ...
                        self.save_histograms_graph(predicted_translations_graph, truth_graph, loss_trans_graph)
    ...
    def log_info(self, loss, loss_rot, loss_trans, loss_trans_graph):
        ...
        disp = [
            ...
            f"Loss Trans graph: {loss_trans_graph.item():.4f}",
            f"LR: {self.lr:e}",  # self.lr  current_lr
    ...
    def save_histograms_graph(self, predicted, truth, loss):
        predicted = predicted.detach().cpu().numpy()
        truth = truth.detach().cpu().numpy()
        ...
        self.writer.add_figure("Translation Graph Visualization", fig, self.iteration)
