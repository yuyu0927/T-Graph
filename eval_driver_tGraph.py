...
- To generate a list of evaluation job commands to run:
    python eval_driver_tGraph.py --list_jobs --output_text_path [path]

- To read all evaluation results and print a nicely formatted report:
    python eval_driver_tGraph.py --list_jobs --output_text_path [path] --results_dir [directory]
...
- To run a single evaluation job for a model given the mode of evaluation, number of frames,
  and CO3D category and order index:
    python eval_driver_tGraph.py --checkpoint_path weights/pairT --mode pairwise \
...
from dataset.co3d_v2_pairT import TEST_CATEGORIES, TRAINING_CATEGORIES
...
DEFAULT_CHECKPOINT_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "weights/pairT"
)

# Change these arguments to suit your needs
OUTPUT_DIRECTORIES = [DEFAULT_CHECKPOINT_PATH]
MODES = ["coordinate_ascent", "cc", "t"]  # "pairwise"
NUM_FRAMES = [2, 3, 4, 5, 6, 7, 8]
CATEGORIES = TEST_CATEGORIES  # TEST_CATEGORIES TRAINING_CATEGORIES
    ...
    with open(output_path, "w") as f:
        for checkpoint in OUTPUT_DIRECTORIES:
            for order in [0, 1, 2, 3, 4]:
                for mode in MODES:  # ["cc", "t"]
                    for num_frames in NUM_FRAMES:
                        for category in CATEGORIES:
                            command = f"python3 eval_driver_tGraph.py --checkpoint_path {checkpoint} --mode {mode} --num_frames {num_frames} --category {category} --sample_num {order}\n"
                            f.write(command)


def checkpoint_report(output_path, results_dir, sample_num):
    with open(output_path, "w") as f1:
        for mode in ["cc", "t"]:  # ["pairwise", "coordinate_ascent"]  ["cc", "t"]
            for num_frames in NUM_FRAMES:
                scores = []
                for category in CATEGORIES:
                    f = open(
                    ...
                    if results != {}:
                        ...
                        acc = np.mean(np.array(angular_errors) < 0.2)  # 15 0.2
                ...
                f1.write(f"{mode} {num_frames}: {avg}" + '\n')
...
def get_parser():
    ...
    parser.add_argument("--sample_num", type=int, default=0)  # 0,1,2,3,4, average 5 results
    return parser
...
if __name__ == "__main__":
    ...
    if args.list_jobs:
        ...
        checkpoint_report(args.output_path, args.results_dir, args.sample_num)  # output_text_path
    else:
        EVAL_FN_MAP = {
