...
from utils.normalize_cameras import first_camera_transform, normalize_cameras, normalize_cameras_1
...
class Co3dDataset(Dataset):
    """
    CO3D sequence dataset.

    Each sample:
        - Selects `num_images` views from a sequence
        - Crops around a jittered square bbox
        - Applies transform (default: ToTensor+Resize+Normalize)
        - Returns images and camera parameters (+ optional normalized cameras)

    Supports:
        - LMDB-backed image/mask loading
        - Optional background masking (white BG)
        - Optional camera normalization / first-camera canonicalization
        - Relative rotation + translation pairs (when normalize_cameras=True)
    """
    ...
    def __init__(
        ...
        self.normalize_cameras = normalize_cameras_flag
        self.first_camera_transform = first_camera_transform_flag
        ...
        # Parsed annotations:
        # self.seq_data[seq_name] -> list of filtered anno dict
        self.seq_data: Dict[str, List[AnnoItem]] = {}
        self.seq_to_category: Dict[str, str] = {}
        self.low_quality_translations: List[str] = []

        self._load_annotations(split_name=split_name)

        self.sequence_list = list(self.seq_data.keys())

        # LMDB cache: open once per category, reused across __getitem__
    ...
    def _load_annotations(self, split_name: str) -> None:
        """
        Read and filter annotation jgz files:
          - drop sequences with len < num_images
          - drop sequences with ridiculous translations
          - keep only keys needed
        """
    ...
    def _get_lmdb_env(self, category: str) -> lmdb.Environment:
        """
        Reuse LMDB env per category (much faster than open/close each sample).
        """
        if category not in self._lmdb_envs:
            env_name = f"lmdb_{category}"
            env_path = osp.join(CO3D_DIR, env_name)
    ...
    def _jitter_bbox(self, bbox_xyxy: np.ndarray) -> np.ndarray:
        ...
        returns jittered square bbox in int (for PIL crop).
        """
        ...
        return np.concatenate((ul, lr))  # [x0,y0,x1,y1]

    @staticmethod
    def _crop_image(image: Image.Image, bbox_xyxy: np.ndarray, white_bg: bool) -> Image.Image:
        """
        Crop with bbox, optionally padding outside area with white background.
        """
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        # Randomly choose `num_images` views
    ...
    def get_data(
        ...
        # 1) Load images (+ optional masks) from LMDB
        ...
        with env.begin(write=False) as txn:
            ...
            for anno in annos:
                ...
                img_bin = txn.get(filepath.encode())
                if img_bin is None:
                    raise FileNotFoundError(f"LMDB missing key (image): {filepath}")
                image = Image.open(io.BytesIO(img_bin)).convert("RGB")

                if self.mask_images:
                    # Compose foreground on white background using mask
                    white = Image.new("RGB", image.size, (255, 255, 255))
                    mask_name = osp.basename(filepath.replace(".jpg", ".png"))
                    mask_key = osp.join(category, sequence_name, "masks", mask_name)

                    mask_bin = txn.get(mask_key.encode())
                    if mask_bin is None:
                        raise FileNotFoundError(f"LMDB missing key (mask): {mask_key}")
                    mask = Image.open(io.BytesIO(mask_bin)).convert("L")
        ...
        # 2) Crop + transform + compute crop_params (for camera-crop conditioning)
        for anno, image in zip(annos, images_pil):
            w, h = image.width, image.height
            bbox = np.array(anno["bbox"])
            bbox_j = self._jitter_bbox(bbox)

            image_crop = self._crop_image(image, bbox_j, white_bg=self.mask_images)
            image_t = self.transform(image_crop) if self.transform is not None else image_crop
        ...
        if self.normalize_cameras:
            ...
            norm_cams, *_ = normalize_cameras(cameras)
            if norm_cams == -1:
            ...
            # translation_pair via pairwise normalization
            perms = get_permutations(len(ids), eval_time=self.eval_time)
            translation_pair = torch.zeros((len(perms), 2, 3), dtype=torch.float32)

            scale_max_update = 0.0
            for k, (i, j) in enumerate(perms):
                annos_pair = [annos[i], annos[j]]
                cams_pair = PerspectiveCameras(
                    focal_length=[a["focal_length"] for a in annos_pair],
                    principal_point=[a["principal_point"] for a in annos_pair],
                    R=[a["R"] for a in annos_pair],
                    T=[a["T"] for a in annos_pair],
                )
                norm_pair, *rest = normalize_cameras_1(cams_pair)
                if norm_pair == -1:
                    raise RuntimeError("normalize_cameras_1 failed: camera scale was 0")

                # unpack last output scale_max if your normalize_cameras_1 returns it last
                scale_max = rest[-1]
                scale_max_update = max(scale_max_update, float(scale_max))

                translation_pair[k] = norm_pair.T  # (2,3)

            if scale_max_update <= 0:
                raise RuntimeError("Invalid scale_max_update <= 0 in translation_pair normalization")

            batch["translation_pair"] = translation_pair / scale_max_update

            if torch.isnan(batch["translation_pair"]).any() or torch.isnan(batch["T"]).any():
                raise RuntimeError(
                    f"NaN detected: category={category}, seq={sequence_name}, ids={ids}"
                )
        else:
            batch["R"] = torch.stack(rotations)
            batch["T"] = torch.stack(translations)

        # 4) Relative rotations (always computed from raw rotations you loaded)
        perms = get_permutations(len(ids), eval_time=self.eval_time)
        rel_R = torch.zeros((len(perms), 3, 3), dtype=torch.float32)
        for k, (i, j) in enumerate(perms):
            rel_R[k] = rotations[i].T @ rotations[j]
        batch["relative_rotation"] = rel_R

        # 5) Images
        batch["image"] = torch.stack(images_tensor) if self.transform is not None else images_tensor
