...
class TranslationRegressor(nn.Module):
    ...
    def forward(
        ...
        assert num_tokens == 2  # should always be 2
        ...
        # input: current image + reference image
        for i in range(num_batches):
            feature1 = features[i][0]
            feature2 = features[i][1]

            fusion1 = torch.cat((feature1, feature2), dim=0)  # reference image: feature2
            fusion2 = torch.cat((feature2, feature1), dim=0)  # reference image: feature1

            features_full[i * num_tokens + 0] = fusion1
            features_full[i * num_tokens + 1] = fusion2
        ...
        translations = self.decode(h).reshape((num_batches, 2, 3))

        bias = torch.FloatTensor([[0, 0, 1], [0, 0, 1]]).to(translations.device)
        translations += bias
