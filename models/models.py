...
from .regressor import TranslationRegressor as TranslationRegressor
from .regressor_pairT import TranslationRegressor as TranslationRegressor_graph
# from .regressor_relativeT import TranslationRegressor as TranslationRegressor_graph
...
class RelPose(nn.Module):
    def __init__(
        ...
        self.translation_regressor_graph = TranslationRegressor_graph(
            num_images=self.num_images,
            feature_dim=self.feature_dim,
            crop_pe_dim=self.crop_pe_dim,
        )
    ...
    def forward(
        ...
        # define translation output matrix
        translations_full = torch.zeros(
            (batch_size, len(permutations), 2, 3),
            device=features.device,
        )
        ...
        for k, (i, j) in enumerate(permutations, start=0):
            ...
            featuresPair_0 = torch.stack([features1, features2], dim=1)
            featuresPair = featuresPair_0.reshape(
            ...
            # predict translation
            translations = self.translation_regressor_graph(featuresPair_0)
            translations_full[:, k, :, :] = translations

        translations = self.translation_regressor(features)
        return queries_all, logits, translations, translations_full
